...
def get_data_set(pos=0):
    dataset = {}
    img_L = np.array(Image.open(get_data_folder(sub_folders[pos]) + 'im2.png').convert('L'))
    img_R = np.array(Image.open(get_data_folder(sub_folders[pos]) + 'im6.png').convert('L'))
    img_result = np.array(Image.open(get_data_folder(sub_folders[pos]) + 'disp6.png').convert('L'))
    dataset['left'] = img_L
    dataset['right'] = img_R
